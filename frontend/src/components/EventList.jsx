        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 transition-opacity duration-300 opacity-100">
          <div className={`w-full max-w-md px-6 py-4 rounded-xl text-black shadow-xl transform transition-all duration-300 ${popup.type === 'success' ? 'bg-green-300 text-white' : popup.type === 'error' ? 'bg-red-300' : 'bg-white'}`}>
            <h3 className="text-2xl font-semibold mb-4 text-center">
              {popup.type === 'success' ? 'Success' : popup.type === 'error' ? 'Error' : 'Notice'}
            </h3>
              ...
              {popup.type === 'confirm' ? (
                <>
                  <button
                    ...
                    className="px-6 py-2 bg-red-600 text-white rounded-lg hover:bg-gray-500 transition duration-300 transform hover:scale-105"
                  >
                    Cancel
                  </button>
                  <button
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="space-x-4">
          <button
            ...
            className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition duration-300 transform hover:scale-105"
          >
            Show My Bookings
          </button>
          <button
            onClick={handleAdminPanel}
            className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition duration-300 transform hover:scale-105"
          >
            Admin Panel
          </button>
        </div>
